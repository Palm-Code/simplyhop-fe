...
export default function CalendarExamplePage() {
  const [events, setEvents] =
    React.useState<CalendarEventType[]>(complexSampleEvents);
  const [filteredEvents, setFilteredEvents] =
    React.useState<CalendarEventType[]>(complexSampleEvents);
  const [categories, setCategories] =
    React.useState<EventCategory[]>(sampleCategories);
  const [reminders, setReminders] = React.useState<EventReminder[]>(
    complexSampleReminders
  );
  const [shares, setShares] =
    React.useState<CalendarShare[]>(complexSampleShares);
  const [currentDate, setCurrentDate] = React.useState(new Date());
  const [selectedDate, setSelectedDate] = React.useState<Date | undefined>(
    new Date()
  );
  const [activeTab, setActiveTab] = React.useState<
    "calendar" | "categories" | "reminders" | "import" | "print" | "share"
  >("calendar");

  // Event detail modal state
  const [isEventDetailModalOpen, setIsEventDetailModalOpen] =
  ...
  const handleUpdateReminder = (
    ...
    setReminders((prev) =>
      prev.map((rem) => (rem.id === reminderId ? { ...rem, ...updates } : rem))
    );
  };
  ...
  const handleCreateShare = (
    share: Omit<CalendarShare, "id" | "createdAt" | "accessCount">
  ) => {
    const newShare = {
      ...share,
      id: `share-${Date.now()}`,
  ...
  if (!process.env.NEXT_PUBLIC_SIMPLY_HOP_CALENDAR_FEATURE || process.env.NEXT_PUBLIC_SIMPLY_HOP_CALENDAR_FEATURE === "false") {
    return null;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">
                Full-featured Google Calendar-like interface with{" "}
                {events.length} complex sample events
              </p>
              <div className="mt-2 flex flex-wrap gap-4 text-xs text-gray-500">
                <span className="bg-gray-100 px-2 py-1 rounded">
                  📅 {events.length} Events across{" "}
                  {Math.ceil(
                    (Math.max(...events.map((e) => e.date.getTime())) -
                      Math.min(...events.map((e) => e.date.getTime()))) /
                      (1000 * 60 * 60 * 24)
                  )}{" "}
                  days
                </span>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
                    ...
                    <div className="space-y-3 text-xs">
                      <div className="bg-white bg-opacity-50 rounded p-2">
                        ...
                        <ul className="space-y-1 text-blue-700">
                          <li>
                            • Overlapping meetings (Today 9:00-9:30 &
                            9:15-10:30)
                          </li>
                          <li>• Multi-day conferences (Days 3-5)</li>
                          <li>
                            • All-day events (Company retreat, Anniversary)
                          </li>
                          <li>• Late night maintenance (23:00-02:00)</li>
                          <li>
                            • Early morning activities (06:30 jog, 05:30 pickup)
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-white bg-opacity-50 rounded p-2">
                        ...
                        <ul className="space-y-1 text-blue-700">
                          <li>
                            • {categories.length} categories with icons & colors
                          </li>
                          <li>
                            • {reminders.filter((r) => r.isActive).length}{" "}
                            active reminders
                        ...
                        <div className="bg-orange-50 border border-orange-200 rounded p-2">
                          ...
                          {conflictingEvents
                            ...
                            .map((conflict, idx) => (
                              ...
                              >
                                • {conflict.date}: {conflict.events.length}{" "}
                                overlapping events
                              </div>
                            ))}
                        </div>
        ...
        {activeTab === "import" && (
          <ImportExport events={events} onImportEvents={handleImportEvents} />
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Month, Week, and Day views with seamless switching
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Move events between dates with intuitive drag and drop
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Find events quickly with text search and color filtering
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Create, edit, and delete events with detailed forms
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Navigate efficiently with keyboard shortcuts and hotkeys
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Quick date navigation with compact sidebar calendar
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">
            Demo Use Cases & Complex Scenarios
          </h2>
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                ...
                <ul className="space-y-2 text-sm text-orange-800">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <div>
                      <strong>9:15-10:30:</strong> Client Presentation Prep
                      (Meeting Room B)
                      <div className="text-xs text-orange-600 mt-1">
                        ⚠️ 15-minute overlap with standup
                      </div>
                    </div>
                  </li>
                </ul>
                <div className="mt-3 text-xs text-orange-600 italic">
                  This demonstrates real-world scheduling conflicts that often
                  occur in busy workplaces.
                </div>
              </div>

              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <h4 className="font-medium text-red-900 mb-2">
                  Next Week&apos;s Double Booking:
                </h4>
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-red-600 mt-1">
                        ⚠️ 1-hour overlap - decision needed
                      </div>
                    </div>
                  </li>
                </ul>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="space-y-2 text-sm text-blue-800">
                  <li>
                    <strong>Early Morning:</strong> 6:30 AM Jog, 5:30 AM Airport
                    Pickup
                  </li>
                  <li>
                    <strong>Standard Business:</strong> 9:00 AM - 5:00 PM
                    meetings
                  </li>
                  <li>
                    <strong>Evening Events:</strong> 6:00 PM Gym, 7:00 PM Family
                    Time
                  </li>
                  <li>
                    <strong>Overnight:</strong> 11:00 PM - 2:00 AM System
                    Maintenance
                  </li>
                  <li>
                    <strong>All-Day:</strong> Company Retreat, Anniversary
                  </li>
                  <li>
                    <strong>Multi-Day:</strong> 3-day Tech Conference
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-green-800">
                  <div className="flex items-center gap-2">
                    <span>💼</span> Work (
                    {events.filter((e) => e.categoryId === "work").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>🏠</span> Personal (
                    {events.filter((e) => e.categoryId === "personal").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>🏥</span> Health (
                    {events.filter((e) => e.categoryId === "health").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>👨‍👩‍👧‍👦</span> Family (
                    {events.filter((e) => e.categoryId === "family").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>📚</span> Education (
                    {events.filter((e) => e.categoryId === "education").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>✈️</span> Travel (
                    {events.filter((e) => e.categoryId === "travel").length})
                  </div>
                  <div className="flex items-center gap-2">
                    <span>🎉</span> Social (
                    {events.filter((e) => e.categoryId === "social").length})
                  </div>
                </div>
              </div>
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <ul className="space-y-1 text-sm text-purple-800">
                  <li>
                    • {reminders.filter((r) => r.isActive).length} Active
                    reminders with different timing
                  </li>
