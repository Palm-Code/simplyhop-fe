...
import { TripSupportActionEnum, TripSupportContext } from "../../context";
import { useGetRidesId } from "../../react_query/hooks";
import { RideDetailCardTripSupport } from "../../components/ride_detail_card";
...
export const RideDetailTripSupport = () => {
  const dictionaries = getDictionaries();
  const searchParams = useSearchParams();
  const rideId = searchParams.get("ride_id");
  const { isLg } = useTailwindBreakpoint();
  const router = useRouter();
  const { state, dispatch } = React.useContext(TripSupportContext);
  useGetRidesId();
  ...
  React.useEffect(() => {
    dispatch({
      type: TripSupportActionEnum.SetDetailRideNotificationData,
      payload: {
  ...
  const handleClose = () => {
    dispatch({
      type: TripSupportActionEnum.SetCompleteRideConfirmationData,
      payload: {
  ...
  const handleClickDeleteRide = () => {
    ...
    dispatch({
      type: TripSupportActionEnum.SetDeleteRideNotificationData,
      payload: {
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <RideDetailCardTripSupport {...filteredData} shadow={false} />
          </div>
          {/* Booking */}
          <div
            className={clsx(
